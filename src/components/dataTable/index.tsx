...
const DataTable = ({ caption, headers, rows, showSearch }: TypeDataTable) => {
    const { data, sort, totalPage, handleSearch, showSortIcon, handleHeaderClick, currentPage,
        setCurrentPage } = useDataTables({ rows })
    ...
    return (
        <React.Fragment>
            <table className="rounded-lg w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <caption className="w-full table-caption  p-5 text-lg font-semibold text-left text-gray-900 bg-white dark:text-white dark:bg-gray-800">
                    {caption}
                    {
                        showSearch && <input onChange={(e) => handleSearch(e.target.value)} placeholder='Search...' className='float-right p-1 rounded-md text-black' type='search' />
                    }
                </caption>
            </table>
            <div className="flex items-center justify-between pt-4" aria-label="Table navigation">
                <ul className="inline-flex -space-x-px text-sm h-8">
                    <li >
                        <button disabled={currentPage === 1} onClick={() => setCurrentPage(currentPage - 1)} className=" disabled:cursor-not-allowed flex items-center justify-center px-3 h-8 ml-0 leading-tight text-gray-500 bg-white border border-gray-300 rounded-l-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">Previous</button>
                    </li>
                    {currentPage} - {totalPage}
                    <li>
                        <button disabled={currentPage >= totalPage} onClick={() => setCurrentPage(currentPage + 1)} className=" disabled:cursor-not-allowed flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 rounded-r-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">Next</button>
                    </li>
                </ul>
            </div>
        </React.Fragment>
